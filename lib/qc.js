...
const STYLE_PROFILES = {
  ...
  // Sprites: standard.
  sprite: {},
  // Characters: thin bodies, articulated limbs → allow lower body ratio, more components.
  // Characters often have anti-aliased edges that trigger false-positive checkerboard detection.
  // Note: sharp (Node.js) reads PNG alpha differently than PIL (Python), causing inconsistent scores.
  // Note: death/hurt states may have higher body ratio (character fills more canvas).
  // Note: enemy_general has large separate components (armor/helmet) → high noise_ratio is legitimate.
  character: {
    minBodyRatio: 0.03,           // 降低：部分敌人组件比例低
    maxBodyRatio: 0.95,           // 允许死亡/受伤状态身体占满画布
    maxNoiseRatio: 0.80,          // 提高容差：角色可能有装备层分离（头盔、护甲）
    maxCheckerboardRatio: 0.10    // 进一步放宽：sharp 与 Python 检测结果不一致
  },
  // Weapons: dense mechanical details create multi-scale patterns, allow higher tolerance.
  // Weapons are thin/elongated objects — allow lower body ratio and higher transparency.
  // Note: weapons often have separate components (grip, barrel, trigger guard) that create
  // multiple connected components. Increase noise tolerance for weapon style profile.
  weapon: {
    maxCheckerboardRatio: 0.10,
    maxNoiseRatio: 0.55,       // 进一步提高容差：泵动霰弹枪有多个分离部件（枪管、握把、击锤）
    minBodyRatio: 0.05,
    maxTransparencyRatio: 0.99
  },
  // Containers/Furniture: textured surfaces (fabric, wood grain) trigger false positives
  // Note: concrete barriers, burnt cars, wooden crates have multiple separated pieces
  // Note: barrier_concrete has noise_ratio=1.60 (debris scattered around main structure)
  container: { maxCheckerboardRatio: 0.10, maxNoiseRatio: 1.80, minBodyRatio: 0.03 },
  // UI: highly transparent panels with decorative borders.
  // UI panels have low body ratio by design - they're backgrounds, not sprites.
  ui: {
    minTransparencyRatio: 0.35,
    maxBodyRatio: 0.95,
    maxEdgeMarginPercent: 0.02,
    minBodyRatio: 0.01,       // UI 面板主体比例可以很低
    maxNoiseRatio: 0.30,      // 允许更多连通分量（装饰性边框）
    maxCheckerboardRatio: 0.05 // 放宽阈值
  },
  // Buildings: wide architectural cross-sections with many windows/doors create multi-scale patterns.
  // Sharp reads PNG alpha differently than PIL, causing false-positive checkerboard detection.
  building: {
    minBodyRatio: 0.05,
    maxNoiseRatio: 0.30,
    maxCheckerboardRatio: 0.08  // 大幅放宽：建筑有大量窗户/门洞等多尺度结构
  },
