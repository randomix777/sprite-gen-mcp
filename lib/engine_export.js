...
export async function exportTexturePacker(args) {
  ...
  if (!image_path || !cell_width) return { success: false, error: 'image_path and cell_width required' };
  ...
  return { success: true, output_path: outPath, frame_count: total, grid: { cols, rows } };
}
...
export async function exportAseprite(args) {
  ...
  return { success: true, output_path: outPath, frame_count: total };
}
