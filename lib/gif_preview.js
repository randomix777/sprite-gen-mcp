...
import { validateInputFile, validateOutputPath } from './path_safety.js';
...
export async function generateGifPreview(args) {
  ...
  // Validate output_path with same checks as other write services (traversal, absolute, UNC, input=output, overwrite, target is directory)
  if (output_path) {
    const outErr = validateOutputPath(output_path, [image_path]);
    if (outErr) return outErr;
    // Target is existing directory
    try {
      const s = (await import('fs')).statSync(path.resolve(output_path));
      if (s.isDirectory()) return err(ErrorCode.INVALID_ARGUMENT, 'Output path is a directory, not a file', { stage: 'validation' });
    } catch (_) {}
  }
