...
async function generateWithAgnes(prompt, config, options) {
  ...
  const body = buildAgnesRequestBody(prompt, model, options);
    ...
    for (const refUrl of options.imageUrls) {
      ...
      try {
        if (fs.existsSync(localPath)) {
          ...
          refImages.push(`data:${mimeType};base64,${buf.toString('base64')}`);
        } else {
          refImages.push(refUrl);
        }
      } catch (_) {
        refImages.push(refUrl);
      }
  ...
  for (let i = 0; i < images.length; i++) {
    if (!images[i].data || images[i].data.startsWith('http')) {
      ...
      if (url) {
        const imgResp = await providerFetch(url, {
          provider: 'agnes',
          stage: 'download',
          timeout: 60000,
...
export function buildAgnesEndpoint(baseUrl) {
  const normalized = String(baseUrl || '').replace(/\/+$/, '');
  return normalized.endsWith('/v1/images/generations')
    ? normalized
    : `${normalized}/v1/images/generations`;
}

export function buildAgnesRequestBody(prompt, model, options = {}) {
  const width = options.width || 1024;
  const height = options.height || 1024;
  const longestEdge = Math.max(width, height);
  const size = longestEdge <= 1024 ? '1K'
    : longestEdge <= 2048 ? '2K'
      : longestEdge <= 3072 ? '3K' : '4K';
  const targetRatio = width / height;
  const ratio = AGNES_RATIOS.reduce((closest, candidate) => (
    Math.abs(Math.log(candidate[1] / targetRatio)) < Math.abs(Math.log(closest[1] / targetRatio))
      ? candidate
      : closest
  ))[0];

  return { model, prompt, size, ratio, extra_body: { response_format: 'url' } };
}
