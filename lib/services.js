...
import { regenerateRejectedAssets } from './regenerate.js';
...
export async function editService(args) {
  ...
  if (!unwrapped || unwrapped.images.length === 0) {
    return err(ErrorCode.PROCESSING_FAILED, 'No images generated', { stage: 'processing' });
  }

  const outPath = output_path || path.join(path.dirname(referencePath), `edited_${session_id}.png`);
  const pathErr = validateOutputPath(outPath, [referencePath]);
  if (pathErr) return pathErr;
  const absPath = saveGeneratedImage(unwrapped.images[0].data, unwrapped.images[0].mimeType, outPath);
...
export async function generateCoverPropService(args) {
  return generateCoverProp(args);
}

export async function auditAssetsService(args) {
  try {
    return await auditAssets(args);
  } catch (e) {
    return err(ErrorCode.PROCESSING_FAILED, e.message, { stage: 'processing', cause: e.stack });
  }
}
...
export async function regenerateRejectedAssetsService(args) {
  return regenerateRejectedAssets(args);
}

// ─── Phased CoverProp services ───────────────────────────────────────────────
