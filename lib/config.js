...
const DEFAULT_CONFIG = {
  ...
  credentials: {
    gemini_flash: { apiKey: null },
    stable_diffusion: { apiKey: null },
    agnes: { apiKey: null }
  }
};
...
export function getConfigSummary() {
  ...
  return {
    ...
    godot: config.godot || DEFAULT_CONFIG.godot,
  };
}

// ─── Godot configuration helpers ──────────────────────────────────────────────

/**
 * Get Godot configuration.
 */
export function getGodotConfig() {
  const config = loadConfig();
  return config.godot || DEFAULT_CONFIG.godot;
}

/**
 * Set Godot configuration.
 */
export function setGodotConfig(settings) {
  const config = loadConfig();
  if (!config.godot) config.godot = { ...DEFAULT_CONFIG.godot };
  Object.assign(config.godot, settings);
  return saveConfig(config);
}
 ...
 */
export function validateGodotConfig(config) {
  const errors = [];
  if (config.executablePath && typeof config.executablePath !== 'string') {
    errors.push('executablePath must be a string');
  }
  if (typeof config.requireForPublish !== 'boolean') {
    errors.push('requireForPublish must be a boolean');
  }
