...
async function auditSingleFile(file, options = {}) {
  const { strict = true, canvas_width, canvas_height, ground_anchor, thresholds } = options;
...
export async function auditAssets(args) {
  ...
  // Audit each image
  const assetResults = [];
  let approved = 0, rejected = 0, reviewRequired = 0, unknown = 0;
  ...
  for (const file of imageFiles) {
    ...
    switch (result.status) {
      case QC_STATUS.APPROVED: approved++; break;
      case QC_STATUS.REJECTED: rejected++; break;
      case QC_STATUS.REVIEW_REQUIRED: reviewRequired++; break;
      default: unknown++;
    }
  }
  ...
  for (const [propId, group] of Object.entries(groups)) {
    if (group.files.intact && group.files.rubble) {
      const consistencyResult = await qcStateConsistency({
        reference_path: group.files.intact.path,
      ...
      if (intactResult && consistencyResult.data) {
        intactResult.state_consistency = {
          reference: group.files.intact.path,
