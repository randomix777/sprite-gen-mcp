...
def fix_weapon(filepath, output_path=None):
    ...
    try:
        ...
        # 使用临时文件避免 Windows 路径问题
        temp_dir = str(Path(tempfile.gettempdir()) / 'sprite_gen_fix')
        os.makedirs(temp_dir, exist_ok=True)
        temp_file = os.path.join(temp_dir, f'temp_{input_path.stem}.png')
        ...
        corners_alpha = [
            np.array(test_img)[0, 0, 3],
            np.array(test_img)[0, -1, 3],
            np.array(test_img)[-1, 0, 3],
            np.array(test_img)[-1, -1, 3],
        ]
        test_img.close()
