...
def process_equipment(filepath):
    """处理装备资产"""
    ...
    try:
        ...
        # 形态学清理 - 强力连接
        alpha_bin = alpha > 0
        alpha_bin = ndimage.binary_opening(alpha_bin, structure=np.ones((13,13)))
        alpha_bin = ndimage.binary_closing(alpha_bin, structure=np.ones((13,13)))
...
def main():
    ...
    # 需要处理的装备文件
    files_to_fix = [
        project_dir / 'assets/equipment/backpacks/medium_assault_pack.png',
        project_dir / 'assets/equipment/chests/light_ballistic_vest.png',
    ]
    
    print(f"处理 {len(files_to_fix)} 个装备资产...\n")
