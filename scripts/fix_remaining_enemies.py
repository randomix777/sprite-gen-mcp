...
DIST_THRESHOLD = 55.0  # 更高阈值，减少过渡像素
BBOX_PADDING = 30
...
def main():
    ...
    enemy_dir = project_dir / 'assets/sprites/enemies'
    
    if args.files:
        files = [enemy_dir / f for f in args.files]
    else:
        files = list(enemy_dir.glob('enemy_*.png'))
    
    print(f"处理 {len(files)} 个敌人角色...\n")
